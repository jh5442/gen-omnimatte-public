...
class Blender(core.View):
  ...
  def render(self,
             ...
             return_layers: Sequence[str] = ("rgba", "backward_flow",
                                             ...
                                             "segmentation"),
             transparent_obj_names: Sequence[str] = (),
             ) -> Dict[str, np.ndarray]:
    ...
    render_settings = self.blender_scene.cycles
    render_settings.max_bounces = 20
    render_settings.transparency_bounces = 20
# --- starts rendering
    if frames is None:
      frames = range(self.scene.frame_start, self.scene.frame_end + 1)

    # only support older blender versions
    # for obj in bpy.data.objects:
    #   if (obj.active_material is not None and
    #       obj.active_material.name == "TransparentMaterial"):
  ...
  @add_asset.register(core.TransparentMaterial)
  @blender_utils.prepare_blender_object
  def _add_asset(self, obj: core.TransparentMaterial):
    ...
    transparent_node = tree.nodes.new(type="ShaderNodeBsdfTransparent")
    tree.links.new(transparent_node.outputs["BSDF"], output_node.inputs["Surface"])
