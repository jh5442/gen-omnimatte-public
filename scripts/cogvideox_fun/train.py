...
def main():
    ...
    transformer3d = CogVideoXTransformer3DModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="transformer",
        use_vae_mask=args.use_vae_mask, shuffle_mask=args.shuffle_mask,
    )
    ...
    if args.transformer_path is not None:
        ...
        if args.use_vae_mask or args.shuffle_mask:
            ...
            new_total_dim = transformer3d.state_dict()['patch_embed.proj.weight'].size(1)
            if new_total_dim > old_total_dim:
                print(f"[DEBUG] The new transformer3d has more channels than the old one.")
                transformer3d.state_dict()['patch_embed.proj.weight'][:, :feat_dim] = state_dict['patch_embed.proj.weight'][:, :feat_dim]
                transformer3d.state_dict()['patch_embed.proj.weight'][:, -feat_dim:] = state_dict['patch_embed.proj.weight'][:, -feat_dim:]
                for i in range(feat_dim, new_total_dim - feat_dim, feat_scale):
                    transformer3d.state_dict()['patch_embed.proj.weight'][:, i:i+feat_scale] = state_dict['patch_embed.proj.weight'][:, feat_dim:-feat_dim]
                state_dict['patch_embed.proj.weight'] = transformer3d.state_dict()['patch_embed.proj.weight']
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "normal":
                        ...
                        if not args.use_vae_mask and not args.shuffle_mask:
                            ...
                            mask = _batch_encode_vae(mask.repeat(1, 1, 3, 1, 1))
